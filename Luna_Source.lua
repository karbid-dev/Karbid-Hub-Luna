...
if _G.NebulaIcons and type(_G.NebulaIcons.GetIcon) == "function" then
	local orig = _G.NebulaIcons.GetIcon
	_G.NebulaIcons.GetIcon = function(self, name, source)
		local res = orig(self, name, source)
		return type(res) == "number" and "http://www.roblox.com/asset/?id=" .. res
	end
end
...
local dragBarCosmetic = dragBar and dragBar.Drag or nil -- -- Button Cosmetic

local Dragger = Main.Drag -- Smart Window Drag
...
MobileSupport.BackgroundTransparency = 1
if MobileSupport and MobileSupport.Interact and MobileSupport.Interact:IsA("TextButton") then
    local oldInteract = MobileSupport.Interact
    local newInteract = Instance.new("ImageButton")
    
    -- Copy essential properties
    newInteract.Name = oldInteract.Name
	newInteract.Size = UDim2.new(1,0,1,0)
	newInteract.AnchorPoint = Vector2.new(0.5, 0.5)
	newInteract.Position = UDim2.new(0.5, 0, 0.5, 0)
    newInteract.BackgroundTransparency = oldInteract.BackgroundTransparency
    newInteract.BackgroundColor3 = oldInteract.BackgroundColor3
    ...
    -- Copy any UIStroke or other children if present
    for _, child in ipairs(oldInteract:GetChildren()) do
        child:Clone().Parent = newInteract
    end
    
    -- Parent the new ImageButton and destroy the old one
    newInteract.Parent = MobileSupport
    oldInteract:Destroy()
...
--------------------------------------------------------------------------------------------------------------------- ⚠️ | Draggable
local function Draggable(Bar, Window, moveDragBar)
    moveDragBar = moveDragBar or false  -- Default to false if not specified
    local Dragging, DragInput, MousePos, FramePos

    local function getDragBar()
        if dragBar and moveDragBar then  -- Only animate if this draggable should affect the dragbar
    ...
    Bar.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            ...
            if moveDragBar then  -- Only animate dragbar if this draggable should affect it
                TweenService:Create(
                    dragBarCosmetic,
    ...
    UserInputService.InputChanged:Connect(function(Input)
        if Input == DragInput and Dragging then
            ...
            if dragBar and moveDragBar then  -- Only move dragbar if this draggable should affect it
                local newDragBarPosition = UDim2.new(
                    FramePos.X.Scale,
                    FramePos.X.Offset + Delta.X,
                    FramePos.Y.Scale,
                    FramePos.Y.Offset + Delta.Y + 240
                )
...
function Luna:CreateWindow(WindowSettings)
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "" and Release or " "

	Navigation.Player.icon.ImageLabel.Image = Players:GetUserThumbnailAsync(
	...
	if WindowSettings.KeySystem then
		...
		Draggable(Dragger, Main, true)
		Draggable(MobileSupport.Interact, MobileSupport, false)
		if dragBar then
			Draggable(dragInteract, Main, true)
		end
		...
		if typeof(WindowSettings.KeySettings.Key) == "string" then
			WindowSettings.KeySettings.Key = { WindowSettings.KeySettings.Key }
		end

		local direc = WindowSettings.KeySettings.SaveInRoot
				and "Luna/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"
			or "Luna/Configurations/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"
		...
		if not Passthrough then
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				if #KeySystem.Input.InputBox.Text == 0 then
					return
				end
	...
	if WindowSettings.KeySystem then
 	...
 	if WindowSettings.LoadingEnabled then
        task.spawn(function()  -- Run loading asynchronously
			LoadingFrame.BackgroundTransparency = 0
            task.wait(0.3) --0.3
            TweenService:Create(
                LoadingFrame.Frame.Frame.Title,
                TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
                { TextTransparency = 0 }
            ):Play()
            TweenService:Create(
                LoadingFrame.Frame.ImageLabel,
                TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
                { ImageTransparency = 0 }
            ):Play()
            task.wait(0.05) --0.05
            TweenService:Create(
                LoadingFrame.Frame.Frame.Subtitle,
                TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
                { TextTransparency = 0 }
            ):Play()
            TweenService
            ...
            TweenService:Create(
                LoadingFrame.Frame.ImageLabel,
                TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out, -1, true, 0),
                { Rotation = 360 }
            ):Play()

			repeat task.wait(0.1) until _G.ScriptLoaded
            ...
            TweenService
                :Create(LoadingFrame.Version, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { TextTransparency = 1 })
                :Play()
            wait(0.3)
            TweenService:Create(
                Main,
            ...
            TweenService:Create(
                Navigation.Player.icon.ImageLabel,
                TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
                { ImageTransparency = 0 }
            ):Play()
            TweenService:Create(
                Navigation.Player.icon.UIStroke,
                TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
                { Transparency = 0 }
            ):Play()
            TweenService:Create(
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		if HomeTabSettings.Icon == 2 then
			HomeTabButton.ImageLabel.Image = _G.NebulaIcons:GetIcon("dashboard", "Material")
		end
		...
		function HomeTab:Activate()
			...
			for _, OtherTabButton in ipairs(Navigation.Tabs:GetChildren()) do
				if
					OtherTabButton.Name ~= "InActive Template"
					and OtherTabButton.ClassName == "Frame"
					and OtherTabButton ~= HomeTabButton
				then
		...
		HomeTabPage.detailsholder.dashboard.Client.Title.Text = (
			isStudio and "Debugging (Studio)" or identifyexecutor()
		) or "Your Executor Does Not Support identifyexecutor."
		for i, v in pairs(HomeTabSettings.SupportedExecutors) do
			...
			if v == identifyexecutor() then
				...
				HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text =
					"Your Executor Isn't Officialy Supported By This Script."
				break
			end
		end
		...
		local function checkFriends()
			if friendsCooldown == 0 then
				...
				HomeTabPage.detailsholder.dashboard.Friends.Offline.Value.Text = tostring(
					friendsInTotal - onlineFriends
				) .. " friends"
		...
		coroutine.wrap(function()
			while task.wait() do
				...
				HomeTabPage.detailsholder.dashboard.Server.MaxPlayers.Value.Text = Players.MaxPlayers
					.. " players can join this server"

				-- Ping
				HomeTabPage.detailsholder.dashboard.Server.Latency.Value.Text = isStudio
						and tostring(math.round((Players.LocalPlayer:GetNetworkPing() * 2) / 0.01)) .. "ms"
				...
				-- Region
				HomeTabPage.detailsholder.dashboard.Server.Region.Value.Text =
					Localization:GetCountryRegionForPlayerAsync(Players.LocalPlayer)
	...
	function Window:CreateTab(TabSettings)
		...
		TabButton.ImageLabel.Image = _G.NebulaIcons:GetIcon(TabSettings.Icon, TabSettings.ImageSource)
		...
		for _, TemplateElement in ipairs(TabPage:GetChildren()) do
			if
				TemplateElement.ClassName == "Frame"
				or TemplateElement.ClassName == "TextLabel" and TemplateElement.Name ~= "Title"
			then
		...
		function Tab:CreateSection(name: string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				TweenService
					:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { BackgroundTransparency = 0.5 })
					:Play()
				TweenService
					:Create(Button.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
					:Play()
				TweenService
				...
				if ButtonSettings.Description ~= nil and ButtonSettings.Description ~= "" then
					TweenService
						:Create(Button.Desc, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
						:Play()
				end
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print(
							"Luna Interface Suite | " .. ButtonSettings.Name .. " Callback Error " .. tostring(Response)
						)
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(
				...
				function ButtonV:Set(ButtonSettings2)
					...
					Button.Title.Text = ButtonSettings.Name
					if
						ButtonSettings.Description ~= nil
						and ButtonSettings.Description ~= ""
						and Button.Desc ~= nil
					then
						Button.Desc.Text = ButtonSettings.Description
					end
			...
			function Section:CreateLabel(LabelSettings)
				...
				TweenService
					:Create(Label.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
					:Play()
				TweenService
					:Create(Label.Text, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
					:Play()
			...
			function Section:CreateParagraph(ParagraphSettings)
				...
				TweenService
					...
					:Play()
				TweenService
					:Create(Paragraph.Text, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
					:Play()

				function ParagraphV:Update()
					Paragraph.Text.Size =
						UDim2.new(Paragraph.Text.Size.X.Scale, Paragraph.Text.Size.X.Offset, 0, math.huge)
					Paragraph.Text.Size = UDim2.new(
						Paragraph.Text.Size.X.Scale,
						Paragraph.Text.Size.X.Offset,
						0,
						Paragraph.Text.TextBounds.Y
					)
					tween(
						Paragraph,
						{
							Size = UDim2.new(
								...
								0,
								Paragraph.Text.TextBounds.Y + 40
							),
						}
					)
				end
			...
			function Section:CreateSlider(SliderSettings, Flag)
				...
				TweenService
					:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { BackgroundTransparency = 0.5 })
					:Play()
				TweenService
					:Create(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
					:Play()
				TweenService
					...
					Loop = RunService.Stepped:Connect(function()
						if SLDragging then
							...
							TweenService:Create(
								...
								TweenInfo.new(0.1, Enum.EasingStyle.Back, Enum.EasingDirection.In, 0, false),
								{
									Size = UDim2.new(
										0,
				...
				local function Set(NewVal, bleh)
					...
					if not Success then
						...
						Slider.Title.Text = "Callback Error"
						print(
							"Luna Interface Suite | " .. SliderSettings.Name .. " Callback Error " .. tostring(Response)
						)
						wait(0.5)
						Slider.Title.Text = SliderSettings.Name
						TweenService:Create(
				...
				Slider.Value:GetPropertyChangedSignal("Text"):Connect(function()
					...
					if SliderSettings.Range[2] < (tonumber(Slider.Value.Text) or 0) then
						Slider.Value.Text = SliderSettings.Range[2]
					end
			...
			function Section:CreateToggle(ToggleSettings, Flag)
				...
				TweenService
					:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { BackgroundTransparency = 0.5 })
					:Play()
				if ToggleSettings.Description ~= nil and ToggleSettings.Description ~= "" then
					TweenService
						:Create(Toggle.Desc, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
						:Play()
				end
				TweenService
					:Create(Toggle.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
					:Play()
				TweenService
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print(
							"Luna Interface Suite | " .. ToggleSettings.Name .. " Callback Error " .. tostring(Response)
						)
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(
				...
				function ToggleV:Set(NewToggleSettings)
					...
					Toggle.Title.Text = ToggleSettings.Name
					if
						ToggleSettings.Description ~= nil
						and ToggleSettings.Description ~= ""
						and Toggle.Desc ~= nil
					then
						Toggle.Desc.Text = ToggleSettings.Description
					end
			...
			function Section:CreateBind(BindSettings, Flag)
				...
				TweenService
					:Create(Bind, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { BackgroundTransparency = 0.5 })
					:Play()
				TweenService
					:Create(Bind.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
					:Play()
				if BindSettings.Description ~= nil and BindSettings.Description ~= "" then
					TweenService
						:Create(Bind.Desc, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
						:Play()
				end
				TweenService:Create(
					Bind.BindFrame,
				...
				TweenService:Create(
					Bind.BindFrame.UIStroke,
					TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
					{ Transparency = 0.3 }
				):Play()
				TweenService:Create(
					Bind.BindFrame.BindBox,
					TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
					{ TextTransparency = 0 }
				):Play()
				...
				UserInputService.InputBegan:Connect(function(input, processed)
					...
					elseif
						BindSettings.CurrentBind ~= nil
						and (input.KeyCode == Enum.KeyCode[BindSettings.CurrentBind] and not processed)
					then -- Test
						local Held = true
						local Connection
				...
				Bind.BindFrame.BindBox:GetPropertyChangedSignal("Text"):Connect(function()
					TweenService:Create(
						...
						{ Size = UDim2.new(0, Bind.BindFrame.BindBox.TextBounds.X + 20, 0, 30) }
					):Play()
				end)
			...
			function Section:CreateInput(InputSettings, Flag)
				...
				TweenService
					:Create(Input, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { BackgroundTransparency = 0.5 })
					:Play()
				TweenService
					:Create(Input.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
					:Play()
				TweenService
					:Create(Input.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
					:Play()
				if descriptionbool then
					TweenService
						:Create(Input.Desc, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
						:Play()
				end
				TweenService:Create(
					Input.InputFrame,
				...
				TweenService:Create(
					Input.InputFrame.UIStroke,
					TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
					{ Transparency = 0.3 }
				):Play()
				TweenService:Create(
					Input.InputFrame.InputBox,
					TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
					{ TextTransparency = 0 }
				):Play()
				...
				Input.InputFrame.InputBox:GetPropertyChangedSignal("Text"):Connect(function()
					if tonumber(InputSettings.MaxCharacters) then
						if (#Input.InputFrame.InputBox.Text - 1) == InputSettings.MaxCharacters then
							Input.InputFrame.InputBox.Text =
								Input.InputFrame.InputBox.Text:sub(1, InputSettings.MaxCharacters)
						end
					end
					TweenService:Create(
						Input.InputFrame,
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function Toggle()
					...
					if opened then
						...
						Dropdown.List.Size = UDim2.new(1, -23, 1, 0)
						Dropdown.OptionHolderBackground.Size = UDim2.new(1, -23, 1, 0)
					end
				end
							...
							SafeCallback(bleh, function()
								if DropdownSettings.MultipleOptions then
									if
										...
										and type(DropdownSettings.CurrentOption) == "table"
									then
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					Dropdown.Title.Text = DropdownSettings.Name
					if
						DropdownSettings.Description ~= nil
						and DropdownSettings.Description ~= ""
						and Dropdown.Desc ~= nil
					then
						Dropdown.Desc.Text = DropdownSettings.Description
					end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact["MouseButton1Click"]:Connect(function()
				...
				if not Success then
					...
					TweenService
						:Create(Button.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 1 })
						:Play()
					Button.Title.Text = "Callback Error"
		...
		function Tab:CreateLabel(LabelSettings)
			...
			TweenService
				:Create(Label.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
				:Play()
			TweenService:Create(Label.Text, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
				:Play()
		...
		function Tab:CreateParagraph(ParagraphSettings)
			...
			function ParagraphV:Update()
				...
				Paragraph.Text.Size =
					UDim2.new(Paragraph.Text.Size.X.Scale, Paragraph.Text.Size.X.Offset, 0, Paragraph.Text.TextBounds.Y)
				tween(
					Paragraph,
					{
						Size = UDim2.new(
		...
		function Tab:CreateSlider(SliderSettings, Flag)
			...
			local function Set(NewVal, bleh)
				...
				if not Success then
					...
					TweenService
						:Create(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 1 })
						:Play()
					Slider.Title.Text = "Callback Error"
		...
		function Tab:CreateToggle(ToggleSettings, Flag)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					TweenService
						:Create(Toggle.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), { Transparency = 1 })
						:Play()
					Toggle.Title.Text = "Callback Error"
		...
		function Tab:CreateBind(BindSettings, Flag)
			...
			TweenService
				:Create(Bind, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { BackgroundTransparency = 0.5 })
				:Play()
			TweenService:Create(Bind.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
				:Play()
			if BindSettings.Description ~= nil and BindSettings.Description ~= "" then
				TweenService
					:Create(Bind.Desc, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
		...
		function Tab:CreateInput(InputSettings, Flag)
			...
			TweenService
				:Create(Input.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { Transparency = 0.5 })
				:Play()
			TweenService:Create(Input.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
				:Play()
			if descriptionbool then
				TweenService
					:Create(Input.Desc, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), { TextTransparency = 0 })
			...
			Input.InputFrame.InputBox:GetPropertyChangedSignal("Text"):Connect(function()
				...
				if not InputSettings.Enter then
					...
					if not Success then
						...
						Input.Title.Text = "Callback Error"
						print(
							"Luna Interface Suite | " .. InputSettings.Name .. " Callback Error " .. tostring(Response)
						)
						wait(0.5)
						Input.Title.Text = InputSettings.Name
						TweenService:Create(
		...
		function Tab:CreateDropdown(DropdownSettings, Flag)
			...
			local function SafeCallback(param, c2)
				...
				if not Success then
					...
					Dropdown.Title.Text = "Callback Error"
					print(
						"Luna Interface Suite | " .. DropdownSettings.Name .. " Callback Error " .. tostring(Response)
					)
					wait(0.5)
					Dropdown.Title.Text = DropdownSettings.Name
					TweenService:Create(
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				Luna.Folder = WindowSettings.ConfigSettings.RootFolder
					.. "/"
	...
	Main.Logo.MouseButton1Click:Connect(function()
		if Navigation.Size.X.Offset == 205 then
			tween(
				Elements.Parent,
				{ Size = UDim2.new(1, -55, Elements.Parent.Size.Y.Scale, Elements.Parent.Size.Y.Offset) }
			)
			tween(
				Navigation,
				{ Size = UDim2.new(Navigation.Size.X.Scale, 55, Navigation.Size.Y.Scale, Navigation.Size.Y.Offset) }
			)
		else
			tween(
				Elements.Parent,
				{ Size = UDim2.new(1, -205, Elements.Parent.Size.Y.Scale, Elements.Parent.Size.Y.Offset) }
			)
			tween(
				Navigation,
				{ Size = UDim2.new(Navigation.Size.X.Scale, 205, Navigation.Size.Y.Scale, Navigation.Size.Y.Offset) }
			)
		end
	end)
	...
	MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		MobileSupport.Visible = false
